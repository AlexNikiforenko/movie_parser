...
def parse_movie_info(items):
    ...
    for item in items:
        ...
        print(f'Обработан {count} фильм из {len(items)}.')
        count += 1
...
def save_file(items, path):
    with open(path, 'w', newline='') as file:
        ...
        rating_average_kp = rating_average_imdb = current_rating_kp = current_rating_imdb = rating_sum_kp = rating_sum_imdb = count_kp = count_imdb = 0
        passed = False
        ...
        for item in items:
            ...
            # rating_sum += float(item['rating_kp']) + float(item['rating_imdb'])
            # count += 1
            # current_rating_average = rating_sum / count
